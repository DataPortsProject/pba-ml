...
from gym_threshold_begone.envs._baseenv import BaseEnv, get_average_last_entries_from_numeric_list


class BPM2020EnvFixedProcesslength(BaseEnv):
    metadata = {'render.modes': ['human']}
    summary_writer = None
    max_process_length = 0.
    max_alpha = 1.
    min_alpha = 0.
    ...
    def compute_reward(self, adapted, cost, done, predicted_duration, planned_duration, reliability, position,
        ...
        if not done:
            ...
            alpha = BPM2020EnvFixedProcesslength.max_alpha - (((BPM2020EnvFixedProcesslength.max_alpha - BPM2020EnvFixedProcesslength.min_alpha) / BPM2020EnvFixedProcesslength.max_process_length) * position)
